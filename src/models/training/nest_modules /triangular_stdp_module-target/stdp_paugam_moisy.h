...
#ifndef stdp_paugam_noisy_synapse_H
#define stdp_paugam_noisy_synapse_H
...
``stdp_paugam_noisy_synapse`` is a connector to create synapses with spike time
...
class stdp_paugam_noisy_synapse : public Connection< targetidentifierT >
{
...
public:
   ...
   */
  stdp_paugam_noisy_synapse( const stdp_paugam_noisy_synapse& ) = default;
  stdp_paugam_noisy_synapse& operator=( const stdp_paugam_noisy_synapse& ) = default;
...
private:
  double triangular_window(double delta_t)
  {
    double unclipped_result = 1.0 * (1 - std::abs(delta_t - 10.0)/10.0);
    return std::max(unclipped_result, 0.);
  }

  double linear_window(double delta_t)
  {
    double unclipped_result = -0.5 * (1 - delta_t/100.0);
    return std::min(unclipped_result, 0.);
  }

  double symmetric_square_window(double delta_t)
  {
    return (delta_t < 20.0 ? 1.0 : -0.25);
  }
...
#define clip(x, lower, upper) ((x) <= (lower) ? (lower) : (x) >= (upper) ? (upper) : (x))
  ...
  {
    double delta_w = lambda_ * std::pow(Wmax_ - w, mu_plus_) * (
      Wmax_ > 0
      ? triangular_window(delta_t)
      : symmetric_square_window(delta_t)
    );
    w += delta_w;
    return clip(w, std::min(0., Wmax_), std::max(0., Wmax_));
  }

  double
  depress_( double w, double delta_t )
  {
    double delta_w = lambda_ * alpha_ * std::pow(w, mu_minus_) * (
      Wmax_ > 0
      ? linear_window(delta_t)
      : symmetric_square_window(delta_t)
    );
    w += delta_w;
    return clip(w, std::min(0., Wmax_), std::max(0., Wmax_));
...
template < typename targetidentifierT >
inline void
stdp_paugam_noisy_synapse< targetidentifierT >::send( Event& e, thread t, const CommonSynapseProperties& )
{
  ...
  while ( start != finish )
  {
    ...
    weight_ = facilitate_( weight_, -1*minus_dt );
  }
  ...
  if ( start != finish )
  {
    ...
    minus_dt = ( finish->t_ + dendritic_delay ) - t_spike;
    weight_ = depress_( weight_, -1*minus_dt );
  }
...
template < typename targetidentifierT >
stdp_paugam_noisy_synapse< targetidentifierT >::stdp_paugam_noisy_synapse()
  : ConnectionBase()
  , weight_( 1.0 )
  , lambda_( 0.1 )
  , alpha_( 1.0 )
...
template < typename targetidentifierT >
void
stdp_paugam_noisy_synapse< targetidentifierT >::get_status( DictionaryDatum& d ) const
{
...
template < typename targetidentifierT >
void
stdp_paugam_noisy_synapse< targetidentifierT >::set_status( const DictionaryDatum& d, ConnectorModel& cm )
{
...
#endif // of #ifndef stdp_paugam_noisy_synapse_H

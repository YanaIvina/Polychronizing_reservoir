...
class PaugamMoisyNetwork(Generic_spiking_transformer):
    ...
    def _create_network(self, testing_mode):
        ...
        internal_indegree_and_outdegree = self.network_parameters['internal_indegree_and_outdegree']
        neuron_ids = exc_reservoir_neuron_ids
        
        # определяем, как сигнал подаётся на нейроны
        nest.Connect(
        ...
        # определяем, как нейроны связаны между собой
        if topology == 'irreg':
